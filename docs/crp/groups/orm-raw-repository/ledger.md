...
| F-030 | MED | NFR-009 — 로그 출력이 요청 event loop 를 막지 않는다(queue listener 수명 관리 포함) | Fix | **Open** | 미착수 — v2 계획서 Wave 5 (ADR-021) | `stop_log_listener()` 가 `listener, _listener = _listener, None` 으로 **손잡이를 먼저 버리고** `listener.stop()` 을 호출한다. stop 이 실패하면 스레드는 살아 있는데 참조가 없어 회수·재시도가 불가능하다. 재현: `queue.Full` 을 주입하면 `stop() → Full` · `_listener is None` · 스레드 생존. stop 실패는 bounded queue 포화 시 실제로 난다 — 표준 `QueueListener.enqueue_sentinel()` 이 `put_nowait` 을 쓰기 때문. **stdlib 가 그 메서드 독스트링에서 timeout 용 오버라이드 지점이라고 직접 지정**하고 있다 |
