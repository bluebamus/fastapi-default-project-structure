...
| F-021 | LOW | MIG-002 / ORM-REP-007 — 호환 이름은 **사용처 0건 확인 후** 제거한다 | Fix | Fixed | `crud_base`·`services_base` 의 `session` property 제거 + `test_session_holding_bases_expose_only_db_session` | Phase 1 의 세션 명명 전환이 남긴 `CRUDBase.session`·`BaseService.session` deprecated 별칭 2개가 **사용처 0건인 채로 살아 있었다**. 명명 계약 테스트가 모듈 레벨 이름만 봐서 **클래스 속성 별칭을 못 봤다** — MIG-002 단계 9(Round 7)가 모듈 이름 5개는 지웠지만 속성 2개는 검사 사각지대에 있었다. 참조 예제로 읽히는 파일에서 두 이름이 공존하면 읽는 사람이 "무엇이 정식인가"를 매번 다시 판단한다. 회귀 테스트를 클래스 속성까지 보도록 확장 |
| F-022 | MED | ADR-014 / 근거 추적성 — 인용한 요구 ID 는 따라갈 수 있어야 한다 | Fix | Fixed | design-baseline REQ-005·ADR-010 이 SCN-RAW-003 을 정식 신설 + 게이트 검사 9 (`review_gate.check_cited_requirement_ids_exist`) | 중단된 세션이 Raw 쓰기 예제를 만들며 **존재하지 않는 `SCN-RAW-003` 을 코드 주석 3곳에서 근거로 인용**했다 — 원본 명세는 SCN-RAW-002 까지다. 더 조용한 쪽은 범위였다: SCN-RAW-002 는 "운영 공개 API 가 **아니어도** 테스트 fixture 에서 검증하라"인데 실제 구현은 **공개 엔드포인트**였고, 그 확대가 design-baseline 어디에도 없었다. 포트(F-019)·해시(F-020)에 이은 **세 번째 dangling reference** 라 사람의 주의력이 아니라 검사로 막는다 |
| F-023 | LOW | ADR-014 — 인용 요구 ID 실재 | **Accept-out-of-scope** | Accepted | `review_gate.LEGACY_UNDECLARED_IDS` + residual-risk R-007 | 검사 9 를 붙이자 이 그룹이 생기기 **전** 검수 라운드의 ID 3건(`ADR-019`·`REQ-008`·`REQ-009`)이 드러났다 — 근거 문서가 이 저장소에 없어 따라갈 수 없다. 코드·문서 8곳이 인용 중이지만 그 라운드의 결정을 복원하는 것은 REQ-005 범위 밖이다. 고치지 않되 **목록을 코드에 박아 늘어나지 않게** 막았다 — 새 코드가 이 예외에 기대면 검사가 실패한다 |
