...
(F-016 · 그리고 그룹 개설 직전의 cp949 디코딩 실패 `22fbb2e`).
...
- **ledger Open Fix 0건** (F-001 ~ F-018 전부 Fixed).
- 마지막 게이트 실행: 전건 통과 · 373 tests · 워킹 트리 클린 (`bfac689`).
...
향후 이 그룹에 새 요청이 오면 Round 8 을 이 아래에 추가한다. `residual-risk.md` 의
