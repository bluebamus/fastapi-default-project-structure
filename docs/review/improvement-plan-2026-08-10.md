...
## P5 — FastAPI 상향 (2026-08-10 완료)

> **결과: 상향 완료.** 아래는 최초 검토에서 드러난 차단 요인 3건과 그 해소 과정이다.
> 최종 상태는 `fastapi 0.141.1`, `pytest 200 passed`, `ruff`/`mypy` 통과.

P1-2 의 안전망이 갖춰져 상향을 실제로 시도했다. 첫 시도는 **9 failed** 였고, 그중
1건이 데이터 정합성 문제라 즉시 올리지 않고 선결 과제부터 처리했다.

| | |
|---|---|
| 대상 | 0.115.14 → **0.141.1** (정식 릴리스 94개, 마이너 26개 라인) |
| 의존성 | starlette 0.46.2 / pydantic 2.13.4 그대로 충족 — **연쇄 상향 불필요** |
| 최초 결과 | **9 failed, 189 passed** → 해소 후 **200 passed** |
...
1. **P1-3** — 커밋을 의존성 teardown 에서 **핸들러 본문**으로 이동. 쓰기 의존성 5개를
   일반 함수로 바꾸고 쓰기 핸들러 13개가 `await service.commit()` 을 직접 호출한다.
   "요청 1개 = 트랜잭션 1개" 는 유지했다. 0.115.x 에서도 유효해 상향과 분리해 선행했다.
   → 0.141.1 에서 커밋 실패가 다시 **5xx** 가 됐다.
2. 라우트 조회를 공개 API 로 전환 — 경로만 필요한 7곳은 `app.openapi()["paths"]`.
   라우트 객체가 필요한 1곳은 중첩 재귀 + **세션 의존성 기준 식별**(경로 문자열이
   버전에 따라 프리픽스 유무가 달라지므로).
3. `Path(example=)` → `examples=`.
4. 핀 상향 + `uv.lock` 갱신.

1번을 건너뛰고 올렸다면 **쓰기 실패가 성공 응답으로 둔갑**했을 것이다.

### 남긴 것

`ORJSONResponse` deprecation. 동작에는 영향이 없고(제거가 아니라 권장 중단),
`default_response_class` 교체는 모든 응답의 직렬화 경로가 바뀌는 별건의 성능
판단이라 이번 상향과 분리했다.
