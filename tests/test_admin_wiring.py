...
(active/passive 저장소는 AppRegistry 가 ``app.features.<name>.admin`` 을 직접 import
...
def test_domain_package_reexports_admin_views(domain: str) -> None:
    ...
    package = importlib.import_module(f"app.features.{domain}")
    views = getattr(package, "admin_views", None)
    assert views, (
        f"app/features/{domain}/__init__.py 가 admin_views 를 재노출하지 않습니다. "
