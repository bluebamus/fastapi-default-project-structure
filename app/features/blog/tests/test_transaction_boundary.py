"""쓰기 도메인의 트랜잭션 경계 회귀 테스트 (2026-08-10 외부 검수 후속 P1-2).

현재 구조: `get_blog_service` 는 writer 세션으로 Service 를 조립해 **반환**만 하고,
쓰기 핸들러 본문이 응답 전에 `await service.commit()` 을 호출한다. 예전에는 의존성이
`yield` 이후에 커밋했는데, 그 경계는 FastAPI 의 yield dependency 종료 코드 실행 시점에
의존해 버전을 올릴 때 조용히 바뀌었다(아래 측정). 여기서 네 가지 성질을 고정해 회귀를 잡는다.
...
P1-3(쓰기 경로의 커밋을 응답 전으로 옮기는 전환)이 필요하다 — 현재 구조가 그 결과다.
...
이 테스트가 정확히 그 판정을 위해 존재한다. 그 뒤 커밋을 핸들러 본문으로 옮겨(P1-3)
4번이 다시 통과한다. 결정 배경: `docs/guides/ARCHITECTURE.md` §3.2
("왜 의존성이 아니라 핸들러인가").
...
async def test_commit_failure_is_not_reported_as_success(tx_client):
    ...
    이 테스트가 빨간불이면 응답이 이미 전송된 뒤에 커밋이 실행된다는 뜻이다 —
    커밋이 핸들러 본문(응답 전)에 있는지 확인한다(ARCHITECTURE §3.2).
    """
    client, calls = tx_client
